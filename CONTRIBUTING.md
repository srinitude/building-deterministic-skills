...
teaches: every change must keep both targets intact — the skill must pass
Hermes validators, and the skills it tells you to generate must stay easy for
weak models to execute.

## Ground rules

1. **Preserve both targets.** Keep the skill valid for Hermes AND keep the
...
5. **Keep source grounding honest.** Claims about agentskills.io or Hermes must
   be backed by `reports/source-grounding/` artifacts or live source. Do not
...
6. **No secrets, no network in scripts.** Validators are stdlib-only and
   offline so they run in clean CI and pass the Hermes security scan.
...
If you have a local Hermes install, also confirm the full pytest gate passes:

```bash
cd ~/.hermes/hermes-agent && ./venv/bin/python -m pytest \
  tests/tools/test_skill_manager_tool.py tests/tools/test_skill_size_limits.py \
  tests/agent/test_skill_utils.py tests/tools/test_skills_guard.py -q
```
